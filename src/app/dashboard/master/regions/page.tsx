...
export default function RegionsPage() {
    ...
    const fetchRegions = async () => {
        try {
            const res = await fetch('/api/master/regions', { cache: 'no-store' })
            if (res.ok) {
    ...
    const handleOpenModal = (region?: Region) => {
        if (region) {
            setFormData({ id: region.id, name: region.name })
            setIsEditing(true)
        } else {
            setFormData({ id: '', name: '' })
            setIsEditing(false)
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault()
        setIsSaving(true)
        const url = isEditing ? `/api/master/regions/${formData.id}` : '/api/master/regions'
    ...
    const handleDelete = async (id: string, name: string) => {
        ...
        try {
            const res = await fetch(`/api/master/regions/${id}`, { method: 'DELETE' })
                        ...
                        {regions.map((reg) => (
                            <tr key={reg.id} className="hover:bg-gray-50">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <button onClick={() => handleOpenModal(reg)} className="text-blue-600 hover:text-blue-900 mr-4">
                                        <Edit className="w-5 h-5" />
                                    </button>
                                    <button onClick={() => handleDelete(reg.id, reg.name)} className="text-red-600 hover:text-red-900">
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white rounded-xl shadow-xl p-8 w-full max-w-md">
                        <h2 className="text-2xl font-bold mb-6">{isEditing ? 'Edit Wilayah' : 'Tambah Wilayah'}</h2>
